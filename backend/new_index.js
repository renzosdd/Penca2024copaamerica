        ...
        if (row) {
            ...
            if ((matchDate - now) <= 30 * 60 * 1000) {
                ...
                db.run(sql, [userId, matchId, goals_team1, goals_team2], function(err) {
                    ...
                    updatePredictedNextRounds(userId); // Actualizar predicciones siguientes
        ...
        const finalAndThirdPlace = () => {
            db.get('SELECT team1, team2, result_team1, result_team2 FROM matches WHERE id = 29', (err, semi1) => {
                if (err) {
                    ...
                    db.run('UPDATE matches SET team1 = ? WHERE id = 31', [loserSemi1], (err) => {
                        if (err) {
                            console.error('Error updating final match:', err.message);
                        }
                    });
                    db.run('UPDATE matches SET team1 = ? WHERE id = 32', [winnerSemi1], (err) => {
                        if (err) {
            ...
            db.get('SELECT team1, team2, result_team1, result_team2 FROM matches WHERE id = 30', (err, semi2) => {
                if (err) {
                    ...
                    db.run('UPDATE matches SET team2 = ? WHERE id = 31', [loserSemi2], (err) => {
                        if (err) {
                            console.error('Error updating final match:', err.message);
                        }
                    });
                    db.run('UPDATE matches SET team2 = ? WHERE id = 32', [winnerSemi2], (err) => {
                        if (err) {
                            console.error('Error updating third place match:', err.message);
                        }
                    });
...
const updatePredictedNextRounds = (userId) => {
    ...
    db.all(sql, [userId, userId], (err, standings) => {
        if (err) {
            console.error('Error fetching predicted standings:', err.message);
            return;
        }
        ...
        for (const [matchId, teams] of Object.entries(predictedQuarterFinals)) {
            const updateQuarterFinalSQL = 'UPDATE predictions SET team1 = ?, team2 = ? WHERE match_id = ? AND user_id = ?';
        ...
        db.get('SELECT * FROM predictions WHERE match_id = 25 AND user_id = ?', [userId], (err, match25) => {
            if (err) return console.error(err.message);
            predictedSemiFinals[29].team1 = match25.goals_team1 > match25.goals_team2 ? match25.team1 : match25.team2;
            updatePredictedSemiFinals(userId);
        });

        db.get('SELECT * FROM predictions WHERE match_id = 26 AND user_id = ?', [userId], (err, match26) => {
            if (err) return console.error(err.message);
            predictedSemiFinals[29].team2 = match26.goals_team1 > match26.goals_team2 ? match26.team1 : match26.team2;
            updatePredictedSemiFinals(userId);
        });

        db.get('SELECT * FROM predictions WHERE match_id = 27 AND user_id = ?', [userId], (err, match27) => {
            if (err) return console.error(err.message);
            predictedSemiFinals[30].team1 = match27.goals_team1 > match27.goals_team2 ? match27.team1 : match27.team2;
            updatePredictedSemiFinals(userId);
        });

        db.get('SELECT * FROM predictions WHERE match_id = 28 AND user_id = ?', [userId], (err, match28) => {
            if (err) return console.error(err.message);
            predictedSemiFinals[30].team2 = match28.goals_team1 > match28.goals_team2 ? match28.team1 : match28.team2;
            updatePredictedSemiFinals(userId);
        });

        const updatePredictedSemiFinals = (userId) => {
            for (const [matchId, teams] of Object.entries(predictedSemiFinals)) {
                if (teams.team1 && teams.team2) {
                    const updateSemiFinalSQL = 'UPDATE predictions SET team1 = ?, team2 = ? WHERE match_id = ? AND user_id = ?';
                    db.run(updateSemiFinalSQL, [teams.team1, teams.team2, matchId, userId], (err) => {
                        if (err) {
                            ...
                            predictedFinalAndThirdPlace(userId);
                        }
                    });
                }
            }
        ...
        const predictedFinalAndThirdPlace = (userId) => {
            db.get('SELECT team1, team2, goals_team1, goals_team2 FROM predictions WHERE match_id = 29 AND user_id = ?', [userId], (err, semi1) => {
                if (err) {
                    ...
                    db.run('UPDATE predictions SET team1 = ? WHERE match_id = 32 AND user_id = ?', [loserSemi1, userId], (err) => {
                        if (err) {
                            console.error('Error updating predicted third place match:', err.message);
                        }
                    });
                    db.run('UPDATE predictions SET team1 = ? WHERE match_id = 31 AND user_id = ?', [winnerSemi1, userId], (err) => {
                        if (err) {
            ...
            db.get('SELECT team1, team2, goals_team1, goals_team2 FROM predictions WHERE match_id = 30 AND user_id = ?', [userId], (err, semi2) => {
                if (err) {
                    ...
                    db.run('UPDATE predictions SET team2 = ? WHERE match_id = 32 AND user_id = ?', [loserSemi2, userId], (err) => {
                        if (err) {
                            console.error('Error updating predicted third place match:', err.message);
                        }
                    });
                    db.run('UPDATE predictions SET team2 = ? WHERE match_id = 31 AND user_id = ?', [winnerSemi2, userId], (err) => {
                        if (err) {
                            console.error('Error updating predicted final match:', err.message);
                        }
                    });
